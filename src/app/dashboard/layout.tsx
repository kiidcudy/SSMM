...
import { formatMoney, isRtl } from "@/lib/site";
...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#0b1220] text-[#e8eefc]" dir={isRtl(locale) ? "rtl" : "ltr"} lang={locale}>
      <header className="border-b border-[#1e2a44] bg-[#111827]">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 px-4 py-3">
          ...
          <div className="flex flex-wrap items-center gap-3 text-sm">
            <DashboardLanguageSwitcher locale={locale} />
            <span className="rounded bg-emerald-500/15 px-3 py-1 font-semibold text-emerald-300">
              {t.dash.yourBalance}: {formatMoney(balance, locale)}
            </span>
            <span className="text-[#93a0b8]">
              {t.dash.account}: {session.username}
            </span>
